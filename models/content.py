...
class TripReport(db.Model):
    ...
    photos = db.relationship(
        "Photo", backref="trip_report", lazy=True, cascade="all, delete-orphan"
    )
    comments = db.relationship(
        "Comment", backref="trip_report", lazy=True, cascade="all, delete-orphan"
    )
    ...
    def can_edit(self, user):
        ...
        return (
            ...
            or user.is_admin()
            or (user.is_trip_leader() and user.id == self.trip.leader_id)
        )
    ...
    def get_recent_reports(limit=10):
        ...
        return (
            TripReport.query.filter_by(is_published=True)
    ...
    def get_featured_reports():
        ...
        return (
            TripReport.query.filter_by(is_published=True, featured=True)
            .order_by(TripReport.created_at.desc())
            .all()
        )
    ...
    def get_reports_by_author(author_id):
        ...
        return (
            TripReport.query.filter_by(author_id=author_id)
            .order_by(TripReport.created_at.desc())
            .all()
        )
...
class Photo(db.Model):
    ...
    def can_edit(self, user):
        ...
        return (
            user.id == self.uploaded_by or user.is_admin() or user.id == self.trip_report.author_id
        )
...
class Comment(db.Model):
    ...
    def get_comments_for_trip(trip_id):
        ...
        return (
            Comment.query.filter_by(trip_id=trip_id, is_approved=True)
    ...
    def get_comments_for_report(trip_report_id):
        ...
        return (
            Comment.query.filter_by(trip_report_id=trip_report_id, is_approved=True)
            .order_by(Comment.created_at)
            .all()
        )
    ...
    def get_comments_for_photo(photo_id):
        ...
        return (
            Comment.query.filter_by(photo_id=photo_id, is_approved=True)
            .order_by(Comment.created_at)
            .all()
        )
...
class HistoricalEvent(db.Model):
    ...
    def get_featured_events(limit=5):
        ...
        return (
            HistoricalEvent.query.filter_by(is_featured=True)
    ...
    def get_events_by_category(category, limit=10):
        ...
        return (
            HistoricalEvent.query.filter_by(category=category)
            .order_by(HistoricalEvent.year.desc())
            .limit(limit)
            .all()
        )
    ...
    def get_recent_events(limit=30):
        ...
        return HistoricalEvent.query.order_by(HistoricalEvent.created_at.desc()).limit(limit).all()
...
class NewsItem(db.Model):
    ...
    def get_current_news(limit=5):
        ...
        return (
            NewsItem.query.filter(
                db.or_(NewsItem.expires_at.is_(None), NewsItem.expires_at > datetime.utcnow())
            )
            .order_by(NewsItem.relevance_score.desc(), NewsItem.news_date.desc())
            .limit(limit)
            .all()
        )
    ...
    def get_todays_news():
        ...
        return (
            NewsItem.query.filter_by(news_date=date.today())
    ...
    def get_news_by_category(category, limit=10):
        ...
        return (
            NewsItem.query.filter_by(category=category)
            .filter(db.or_(NewsItem.expires_at.is_(None), NewsItem.expires_at > datetime.utcnow()))
            .order_by(NewsItem.relevance_score.desc())
            .limit(limit)
            .all()
        )
    ...
    def cleanup_expired_news():
        ...
        expired_items = NewsItem.query.filter(NewsItem.expires_at < cutoff_date).all()
...
class DailyNews(db.Model):
    ...
    __table_args__ = (db.Index("idx_daily_news_date", "news_date"),)
