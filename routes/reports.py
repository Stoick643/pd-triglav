...
def list_reports():
    ...
    completed_trips = (
        Trip.query.filter(Trip.status.in_(["completed", "announced"]))
        .order_by(Trip.trip_date.desc())
        .all()
    )
    form.trip_id.choices = [("", "Vsi izleti")] + [
        (trip.id, f"{trip.title} ({trip.trip_date.strftime('%d.%m.%Y')})")
...
def view_report(report_id):
    ...
    report = (
        TripReport.query.options(joinedload(TripReport.trip), joinedload(TripReport.photos))
    ...
    if not report.is_published:
        if not current_user.is_authenticated or (
            current_user.id != report.author_id and not current_user.is_admin()
        ):
            abort(404)
    ...
    return render_template(
        "reports/detail.html", report=report, comments=comments, comment_form=comment_form
    )
...
def create_report(trip_id=None):
    ...
    if trip_id:
        ...
        participation = TripParticipant.query.filter_by(
            trip_id=trip_id, user_id=current_user.id, status=ParticipantStatus.CONFIRMED
        ).first()
        ...
        existing_report = TripReport.query.filter_by(
            trip_id=trip_id, author_id=current_user.id
        ).first()
    ...
    if current_user.is_admin():
        ...
        participations = TripParticipant.query.filter_by(
            user_id=current_user.id, status=ParticipantStatus.CONFIRMED
        ).all()
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if is_published:
                ...
                flash(
                    f'Poročilo "{report.title}"{photo_msg} je bilo uspešno objavljeno!', "success"
                )
            else:
    ...
    return render_template(
        "reports/create.html", form=form, trip=trip, available_trips=available_trips
    )
...
def edit_report(report_id):
    ...
    report = (
        TripReport.query.options(joinedload(TripReport.trip)).filter_by(id=report_id).first_or_404()
    )
...
def add_comment(report_id):
    ...
    if (
        ...
        and current_user.id != report.author_id
...
def my_reports():
    ...
    reports = (
        TripReport.query.options(joinedload(TripReport.trip))
    ...
    return render_template(
        "reports/my_reports.html", reports=reports, available_trips=available_trips
    )
