...
def get_daily_mountaineering_news_for_homepage():
    """Get daily news for homepage - uses database cache only"""
    from models.content import DailyNews, db
    ...
    try:
        ...
        # If no cached news, return empty list (don't block startup)
        # Content will be generated by scheduled task at 6 AM or admin refresh
        return []
    except Exception as e:
        # Ensure we rollback on any error
        db.session.rollback()
        current_app.logger.error(f"Error getting daily news for homepage: {e}")
        return []
