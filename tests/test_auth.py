...
def test_register_validation_errors(client):
    ...
    response = client.post(
        "/auth/register",
        data={"name": "", "email": "test@test.com", "password": "pass", "password_confirm": "pass"},
    )
...
def test_login_invalid_credentials(client, test_users):
    ...
    # Wrong password
    response = client.post(
        "/auth/login", data={"email": "member@test.com", "password": "wrongpassword"}
    )
    assert "Napačen email ali geslo".encode("utf-8") in response.data

    # Non-existent user
    response = client.post(
        "/auth/login", data={"email": "nonexistent@test.com", "password": "password"}
    )
    assert "Napačen email ali geslo".encode("utf-8") in response.data
...
def test_pending_user_dashboard(client, app, test_users):
    ...
    response = client.post(
        "/auth/login", data={"email": "pending@test.com", "password": "pendingpass"}
    )
