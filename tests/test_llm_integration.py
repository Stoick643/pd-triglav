...
class TestLLMProviderIntegration:
    ...
    def test_moonshot_chat_completion(self):
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a test assistant. Always respond with valid JSON.",
            },
            {
                "role": "user",
                "content": 'Generate JSON: {"test": "success", "provider": "moonshot", "timestamp": "2025-01-01"}',
            },
        ]
    ...
    def test_deepseek_chat_completion(self):
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": 'Generate JSON: {"test": "success", "provider": "deepseek", "timestamp": "2025-01-01"}',
            },
        ]
...
class TestProviderManagerIntegration:
    ...
    def test_provider_fallback_logic(self):
        ...
        result = manager.chat_completion_with_fallback(
            messages=messages, use_case="historical", response_format={"type": "json_object"}
        )

        assert isinstance(result, dict), "Result should be a dictionary"
        assert (
            "fallback_test" in result or "fallback" in result
        ), "Should have test result or fallback indicator"
...
class TestLLMServiceIntegration:
    ...
    def test_llm_service_initialization(self):
        ...
        assert (
            len(service.provider_manager.get_available_providers()) > 0
    ...
    def test_historical_event_generation(self):
        ...
        assert (
            1700 <= event_data["year"] <= current_year
        ), f"Year should be reasonable (1700-{current_year})"
        ...
        assert (
            event_data["category"] in valid_categories
        ), f"Category should be one of {valid_categories}"
