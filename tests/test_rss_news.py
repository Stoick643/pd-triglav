...
class TestRSSFeedParser:
    ...
    def mock_rss_data(self):
        ...
        return {
            ...
            "feed": {"title": "PlanetMountain News", "link": "https://planetmountain.com"},
        }
    ...
    def test_extract_source_name_from_url(self, rss_parser):
        ...
        assert (
            rss_parser._extract_source_name("https://planetmountain.com/rss.xml")
            == "planetmountain"
        )
...
class TestClimbingNewsAggregator:
    ...
    def test_combine_rss_and_newsapi_sources(
        self, aggregator, mock_rss_articles, mock_newsapi_articles
    ):
        """Test merging RSS feeds with existing NewsAPI"""
        empty_scraping_articles = []
        combined = aggregator.combine_sources(
            mock_rss_articles, empty_scraping_articles, mock_newsapi_articles
        )
    ...
    def test_source_priority_weighting(self, aggregator, mock_rss_articles, mock_newsapi_articles):
        ...
        rss_scores = [
            article["relevance_score"] for article in scored if article["source"] != "newsapi"
        ]
        newsapi_scores = [
            article["relevance_score"] for article in scored if article["source"] == "newsapi"
        ]
...
class TestWebScrapingParser:
    ...
    def test_date_parsing_various_formats(self, web_scraper):
        ...
        elem1 = BeautifulSoup(
            '<time datetime="2025-01-29T10:00:00Z">Jan 29</time>', "html.parser"
        ).find("time")
        result1 = web_scraper._parse_date(elem1)
        assert "2025-01-29T10:00:00Z" in result1

        # Test text content
        elem2 = BeautifulSoup('<span class="date">January 29, 2025</span>', "html.parser").find(
            "span"
        )
...
class TestWebScrapingIntegration:
    ...
    def test_web_scraping_relevancy_scoring(self, aggregator):
        ...
        webscraping_score = next(
            a["relevance_score"] for a in scored if a["source_type"] == "webscraping"
        )
