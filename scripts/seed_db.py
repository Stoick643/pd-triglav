...
def seed_trips():
    ...
    for trip_data in trips_to_create:
        ...
        existing_trip = Trip.query.filter_by(
            title=trip_data["title"], trip_date=trip_data["trip_date"]
        ).first()
...
def print_trips():
    ...
    for trip in trips:
        status_icon = (
            "📅"
            if trip.status == TripStatus.ANNOUNCED
            else "✅" if trip.status == TripStatus.COMPLETED else "❌"
        )
        print(
            f"{status_icon} {trip.title:<30} | {trip.trip_date} | {trip.difficulty.slovenian_name} | Vodnik: {trip.leader.name}"
        )
